...
async function readJson(response: Response) {
  ...
  if (!contentType.includes("application/json")) throw new Error(response.status === 404 ? "Las rutas de Meta todavía no están publicadas en Vercel. Vuelve a desplegar la carpeta app/api/meta." : "El servidor respondió con un formato inesperado (" + response.status + ").");
...
export function MetaConnections() {
  ...
  async function connectMeta() {
    ...
    try {
      ...
      setMessageType("error"); setMessage(error instanceof Error ? error.message : "No fue posible iniciar la conexión con Meta."); setConnectingMeta(false);
    }
  }
  ...
  async function connectWhatsapp() {
    ...
    try {
      ...
      if (!document.querySelector("script[data-meta-sdk]")) {
        await new Promise<void>((resolve, reject) => { const script = document.createElement("script"); script.src = "/api/meta/facebook-sdk";; script.async = true; script.defer = true; script.dataset.metaSdk = "true"; script.onload = () => resolve(); script.onerror = () => reject(new Error("No se pudo cargar el acceso de Meta")); document.body.appendChild(script); });
      }
      ...
      window.FB.login(async response => {
        try {
          ...
          if (!code) throw new Error("La autorización fue cancelada o Meta no devolvió el código.");
          const session = await waitForEmbeddedSession();
          if (!session?.wabaId) throw new Error("Meta no devolvió la cuenta de WhatsApp seleccionada.");
          ...
          const result = await readJson(completed);
          if (!completed.ok) throw new Error(result.error || "No se pudo guardar la conexión.");
  ...
  async function saveWhatsapp(event: FormEvent) {
    ...
    try {
      ...
      setMessageType("success"); setMessage("WhatsApp conectado correctamente."); setShowWhatsapp(false); setForm({ displayName: "", phoneNumberId: "", businessAccountId: "", accessToken: "" }); await load();
  ...
  async function disconnect(id: string) {
    ...
    try { const response = await fetch("/api/meta/connections?id=" + id, { method: "DELETE" }); const body = await readJson(response); if (!response.ok) throw new Error(body.error || "No fue posible desconectar el canal."); await load(); }
  ...
  return <article className="panel meta-connections">
    <div className="meta-connections-head"><div className="meta-heading"><span className="meta-symbol">M</span><div><p className="eyebrow">META EN PRODUCCIÓN</p><h3>Canales de mensajería</h3><p>Conecta las cuentas propias de esta empresa. Los tokens se cifran antes de guardarse y nunca se muestran nuevamente.</p></div></div><div className="meta-connect-actions"><button type="button" className="primary-button" onClick={connectMeta} disabled={connectingMeta}>{connectingMeta ? "Abriendo Meta…" : "Conectar Facebook Messenger"}</button><button type="button" className="secondary-button" onClick={connectWhatsapp} disabled={connectingWhatsapp}>{connectingWhatsapp ? "Conectando WhatsApp…" : "Conectar WhatsApp"}</button><button type="button" className="ghost-button" onClick={() => setShowWhatsapp(value => !value)}>{showWhatsapp ? "Cerrar conexión manual" : "Conexión manual avanzada"}</button></div></div>
    ...
    {showWhatsapp && <form className="whatsapp-connect" onSubmit={saveWhatsapp}><label><span>Nombre visible</span><input required value={form.displayName} onChange={event => setForm(value => ({ ...value, displayName: event.target.value }))} placeholder="WhatsApp de la empresa" /></label><label><span>Phone Number ID</span><input required inputMode="numeric" value={form.phoneNumberId} onChange={event => setForm(value => ({ ...value, phoneNumberId: event.target.value }))} placeholder="Ej. 1270090499526634" /></label><label><span>WhatsApp Business Account ID</span><input inputMode="numeric" value={form.businessAccountId} onChange={event => setForm(value => ({ ...value, businessAccountId: event.target.value }))} placeholder="Ej. 3540045776153616" /></label><label><span>Token permanente</span><input required type="password" autoComplete="new-password" value={form.accessToken} onChange={event => setForm(value => ({ ...value, accessToken: event.target.value }))} placeholder="••••••••••••••••" /><small>Se cifrará al guardar.</small></label><div className="whatsapp-actions"><button className="primary-button" disabled={loading}>{loading ? "Conectando…" : "Guardar conexión"}</button><button type="button" className="ghost-button" onClick={() => setShowWhatsapp(false)}>Cancelar</button></div></form>}
    <div className="connection-list">{connections.length ? connections.map(item => <div className="connection-row" key={item.id}><span className={"channel-logo " + item.provider}>{item.provider === "whatsapp" ? "W" : item.provider === "instagram" ? "◎" : "f"}</span><div><strong>{item.settings?.display_name || labels[item.provider]}</strong><small>{labels[item.provider]} · {item.external_account_id}</small></div><span className="connection-active">Conectado</span><button className="danger-button" onClick={() => disconnect(item.id)}>Desconectar</button></div>) : <div className="meta-empty"><span>↗</span><strong>Sin canales conectados</strong><p>Conecta Facebook, Instagram o WhatsApp para recibir sus mensajes en la bandeja unificada.</p></div>}</div>
