  ...
  async function connectWhatsapp(){
    ...
    try{
      ...
      if(!/^\d+$/.test(appId)||!/^\d+$/.test(configId)){
        throw new Error("La configuración de WhatsApp Embedded Signup no está completa en Vercel. Verifica META_APP_ID y META_WHATSAPP_CONFIG_ID.");
      }
      ...
      // Facebook's SDK expects a regular callback, not an async function.
      // Run the asynchronous completion flow inside a detached promise.
      window.FB.login(response=>{void (async()=>{
        try{
        ...
        // ESU v4 stores the products, assets and permissions in the Meta
        // configuration. Do not add legacy scope/redirect parameters here:
        // Meta can otherwise fall back to the generic Facebook Login dialog.
        extras:{setup:{}}
      });
