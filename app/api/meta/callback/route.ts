...
export async function GET(request: Request) {
  ...
  const finish = (status: string) => NextResponse.redirect(new URL(`/dashboard?view=configuracion&meta=${encodeURIComponent(status)}`, url.origin));
  ...
  try {
    ...
    const accounts = await graphRequest("me/accounts?fields=id,name,access_token&limit=100", userToken);
    const pages = accounts.data || [];
    if (!pages.length) throw new Error("Meta no devolvió ninguna página administrada. Edita la autorización y selecciona la página correcta.");
